...
void usage(const char *name)
{
    ...
    fprintf(stderr, " -c <client certificate> Client certificate in PEM format.                default: none\n");
    fprintf(stderr, " -k <private client key> Private key that signed the client certificate.  default: none\n");
    fprintf(stderr, " -t <server certificate> Public server certificate.                       default: none\n");
    fprintf(stderr, "\nExample: %s -s rfbtv://localhost -a webkit:http://activevideo.com\n", name);
}

/// \todo Add implementation to read the MAC address (AKA Ethernet
///       hardware address) or a unique serial number.
///
/// \retval Zero terminated string that uniquely identifies this STB.
///
...
void setup_client_context(const std::string &base_store_path, const std::string &ca_client_path, const std::string &private_key_path, const std::string &ca_path)
{
...
int main(int argc, char *argv[])
{
    ...
    int opt;
    while ((opt = getopt(argc, argv, "hs:a:b:c:k:t:")) != -1) {
        switch (opt) {
        case 's':
            server = optarg;
            break;
